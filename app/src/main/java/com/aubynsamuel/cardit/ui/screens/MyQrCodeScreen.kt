...
fun MyQrCodeScreen(profileViewModel: ProfileViewModel, navController: NavHostController) {
    ...
    LaunchedEffect(userProfile) {
        val contactData = userProfile.toJsonString()

        if (contactData.isNotBlank()) {
            try {
                qrBitmap = generateQrCode(contactData)
            } catch (e: Exception) {
                showToast("QR Code generation failed: ${e.message}", context)
            }
        }
    }
